<template>
  <div class="fixed bottom-4 z-50 inset-x-0 pointer-events-none">
    <div class="mx-auto pointer-events-auto px-4 flex justify-center">
      <TooltipProvider>
        <div
          class="dock flex items-center gap-3 max-w-[90vw] md:max-w-xl bg-card dark:bg-card border border-border dark:border-border rounded-md shadow-lg h-12 px-3 sm:px-4 text-foreground "
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="h-6 w-px bg-border/60 mx-1 hidden sm:block"></div>

            <Tooltip>
              <TooltipTrigger as-child>
                <div class="hidden sm:flex items-center gap-2 cursor-pointer" @click="showWordCountDialog = true">
                  <Hash class="w-4 h-4 text-muted-foreground" aria-hidden="true" />
            ...
            <Tooltip>
              <TooltipTrigger as-child>
                <Button type="button" @click="changeZoom(-25)"
                  class="flex items-center justify-center w-8 h-8 rounded-md bg-muted/50 dark:bg-muted/50 border border-border dark:border-border text-foreground hover:bg-accent"
                  aria-label="Zoom out">
            ...
            <Popover>
              ...
              <PopoverContent
                ...
                <div class="flex flex-col">
                  <button
                    class="text-left px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-foreground-100 font-medium"
                    @click="selectZoom('fit')">
                    Fit
                  </button>
                  <div class="border-t my-1 border-border" />
                  <button v-for="opt in zoomOptions" :key="opt"
                    class="text-left px-2 py-1 rounded hover:bg-accent text-foreground" @click="selectZoom(opt)">
                    {{ opt }}%
                  </button>
                </div>
              </PopoverContent>
