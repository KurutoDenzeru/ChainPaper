<template>
    <div :class="[
        'fixed top-0 left-0 h-full w-fit bg-white/95 backdrop-blur-md border-r border-gray-200 shadow-lg z-40 transform transition-transform duration-300 ease-in-out',
        ...
        <div class="flex flex-col h-full">
            ...
            <div class="p-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="sm" @click="$emit('close')" class="h-8 w-8 p-0 cursor-pointer">
                        <X class="w-4 h-4" />
                    </Button>
                </div>
            </div>
            ...
            <Tabs v-model="activeTab" class="flex-1 flex flex-col">
                <TabsList class="grid w-auto grid-cols-3 mx-4 mt-4">
                    <TabsTrigger value="toc" class="text-sm cursor-pointer">
                        <BookOpen class="w-4 h-4 mr-2" />
                        Table of Contents
                    </TabsTrigger>
                    <TabsTrigger value="comments" class="text-sm cursor-pointer">
                        <MessageSquare class="w-4 h-4 mr-2" />
                        Comments
                    </TabsTrigger>
                    <TabsTrigger value="authorship" class="text-sm cursor-pointer">
                ...
                <div class="flex-1 overflow-hidden">
                    ...
                    <TabsContent value="toc" class="flex-1 m-0 p-4 overflow-y-auto">
                        <div class="space-y-2">
                            <div v-for="entry in documentIndex.tableOfContents" :key="entry.id" :class="[
                    ...
                    <TabsContent value="authorship" class="flex-1 m-0 p-4 overflow-y-auto">
                        <div class="space-y-4">
                            ...
                            <div v-if="authorshipProof" class="p-4 bg-green-50 rounded-lg border border-green-200">
                                ...
                                <div class="space-y-2 text-sm">
                                    <div>
                                        ...
                                        <code
                                            class="block text-xs bg-green-100 p-2 rounded mt-1 break-all">{{ authorshipProof.hash }}</code>
                                    </div>
                                    <div class="flex justify-between">
                            ...
                            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                ...
                                <div class="space-y-2">
                                    <div v-for="contributor in documentMetadata.contributors" :key="contributor.id"
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="text-xs text-gray-500">{{ contributor.contributions }}
                                                contributions</div>
                                        </div>
                                    </div>
                                </div>
    ...
    <!-- Backdrop -->
    <div v-if="isVisible" class="fixed inset-0 bg-black/20 backdrop-blur-sm z-30" @click="$emit('close')" />
</template>
