  ...
  <HoverCard :open-delay="0" :close-delay="200">
    ...
    <HoverCardContent class="w-auto p-3">
      <div class="space-y-3">
        ...
        <!-- Grid selector -->
        <div class="space-y-2">
          <div class="grid gap-1 p-2 border rounded-md bg-gray-50"
        ...
        <div class="border-t pt-3 space-y-2">
          <div class="text-sm font-medium text-gray-700">Custom Size</div>
          <div class="flex items-center gap-2">
            <div class="flex items-center gap-1">
              <label class="text-sm text-gray-600">Rows:</label>
              <input v-model.number="customRows" type="number" min="1" max="20"
                class="w-12 px-1 py-0.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div class="flex items-center gap-1">
              <label class="text-sm text-gray-600">Cols:</label>
              <input v-model.number="customCols" type="number" min="1" max="10"
                class="w-12 px-1 py-0.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <Button size="sm" variant="outline" class="h-6 px-2 text-sm" @click="insertTable(customRows, customCols)">
              Insert
            </Button>
          </div>
        </div>
...
<script setup lang="ts">
  ...
  function insertTable(rows: number, cols: number) {
    ...
    emit('insertTable', validRows, validCols, !!includeHeader.value)
