<template>
  <div class="flex items-center justify-between px-4 sm:px-2 py-1 rounded-xl bg-white border border-gray-200 gap-1 sm:gap-2 overflow-x-auto shadow-sm">
    ...
    <div class="hidden lg:flex gap-1 flex-1 items-center">
      ...
      <!-- Basic Formatting -->
      <Button variant="ghost" size="sm" :class="['h-8 w-8 p-0', { 'bg-gray-100': active.bold }]" @click="$emit('format-bold')">
        <Bold class="w-4 h-4" />
      </Button>
      <Button variant="ghost" size="sm" :class="['h-8 w-8 p-0', { 'bg-gray-100': active.italic }]" @click="$emit('format-italic')">
        <Italic class="w-4 h-4" />
      </Button>
      <Button variant="ghost" size="sm" :class="['h-8 w-8 p-0', { 'bg-gray-100': active.underline }]" @click="$emit('format-underline')">
        <Underline class="w-4 h-4" />
      </Button>
      <Button variant="ghost" size="sm" :class="['h-8 w-8 p-0', { 'bg-gray-100': active.strike }]" @click="$emit('format-strikethrough')">
      ...
      <!-- Lists -->
      <Button variant="ghost" size="sm" :class="['h-8 w-8 p-0', { 'bg-gray-100': active.bullet }]" @click="$emit('toggle-bullet-list')">
        <List class="w-4 h-4" />
      </Button>
      <Button variant="ghost" size="sm" :class="['h-8 w-8 p-0', { 'bg-gray-100': active.ordered }]" @click="$emit('toggle-ordered-list')">
    ...
    <div class="hidden md:flex lg:hidden gap-1 flex-1 items-center">
      ...
      <!-- Mode toggle -->
      <Button variant="ghost" size="sm" class="h-8 px-2" @click="toggleMode">
        <span class="text-xs font-medium">{{ mode==='source' ? 'Reader' : 'Source' }}</span>
      </Button>
    </div>
    ...
    <div class="flex md:hidden gap-1 flex-1 items-center">
      ...
      <!-- Mode toggle -->
      <Button variant="ghost" size="sm" class="h-8 px-2" @click="toggleMode">
        <span class="text-xs font-medium">{{ mode==='source' ? 'R' : 'S' }}</span>
      </Button>
    </div>
          ...
          <div class="space-y-1">
            ...
            <div class="md:hidden space-y-1">
              ...
              <div class="grid grid-cols-4 gap-1">
                <Button variant="ghost" size="sm" :class="['h-8', { 'bg-gray-100': active.underline }]" @click="$emit('format-underline')">
                  <Underline class="w-4 h-4" />
                </Button>
                <Button variant="ghost" size="sm" :class="['h-8', { 'bg-gray-100': active.strike }]" @click="$emit('format-strikethrough')">
                  <Strikethrough class="w-4 h-4" />
                </Button>
            ...
            <div class="space-y-1">
              ...
              <div class="grid grid-cols-8 gap-1">
                <button v-for="c in textColors.slice(0,8)" :key="c" class="w-6 h-6 rounded border border-gray-200 hover:scale-110 transition-transform" :style="{ backgroundColor: c }" @click="$emit('set-text-color', c)"></button>
              </div>
              <div class="grid grid-cols-8 gap-1 mt-1">
                <button v-for="c in highlightColors.slice(0,8)" :key="c" class="w-6 h-6 rounded border border-gray-200 hover:scale-110 transition-transform" :style="{ backgroundColor: c }" @click="$emit('set-highlight', c)"></button>
              </div>
            </div>
          </div>
        </PopoverContent>
...
import { Bold, Italic, Underline, Strikethrough, Type, Highlighter, Undo2, Redo2, List, ListOrdered, AlignLeft, AlignCenter, AlignRight, AlignJustify, Link, Image as ImageIcon, Code2, Table, Minus, Plus, Sidebar, Search, MoreHorizontal } from 'lucide-vue-next'
