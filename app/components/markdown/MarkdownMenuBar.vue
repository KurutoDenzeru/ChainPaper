<template>
  <div>
    <!-- Custom Markdown MenuBar (similar to the one in attachment) -->
  ...
  @update:open="(v) => { showExportDialog = v; if (!v) exportInitialFormat = null }"
    />
...
// which export format was requested from the menu (optional)
...
function handleMenuEmit(item: any) {
  ...
  if (item.emit === 'save-document') {
    // open the save dialog rather than calling a store save helper
    showSaveDialog.value = true
    return
  }

  if (item.emit === 'export-markdown' || item.emit === 'export-html' || item.emit === 'export-pdf') {
    // choose initial format based on which submenu was clicked
...
defineExpose({
  openWordCountDialog: () => { showWordCountDialog.value = true },
  openAuthProofDialog: () => { showAuthProofDialog.value = true },
  openExportDialog: () => { showExportDialog.value = true },
  openSaveDialog: () => { showSaveDialog.value = true }
})
</script>
